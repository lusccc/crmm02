...
from crmm.arguments import TextModelArguments, MultimodalDataArguments, CrmmTrainingArguments
...
def main(text_model_args: TextModelArguments,
    ...
    # @@@@ 3. MODEL
    tokenizer = AutoTokenizer.from_pretrained(pretrained_model_name_or_path=text_model_args.text_model_name,
                                              cache_dir=text_model_args.cache_dir,
                                              local_files_only=text_model_args.load_hf_model_from_cache)
    default_model_config_params = dict(n_labels=n_labels,
                                       ...
                                       max_text_length=text_model_args.max_seq_length,
                                       loss_weights=training_args.loss_weights,
                                       text_model_cache_dir=text_model_args.cache_dir,
                                       text_model_name=text_model_args.text_model_name,
                                       text_model_local_files_only=text_model_args.load_hf_model_from_cache,
                                       load_hf_pretrained=True)
    if task == 'pretrain':
        model_config = CLNCPConfig(**default_model_config_params,
                                   freeze_text_params=text_model_args.freeze_text_params,
                                   pretrained=False)
    ...
    elif task == 'conventional_multimodal_classification':
    ...
    get_trainer = lambda model: Trainer(
        ...
        data_collator=MultimodalPretrainCollator(tokenizer, text_model_args.max_seq_length)
        if task in ['pretrain', 'continue_pretrain'] else
        MultimodalClassificationCollator(tokenizer,
                                         text_model_args.max_seq_length,
                                         # note bad or good label is split with @
                                         data_args.natural_language_labels.split('@')),
    )
    trainer = get_trainer(model)
    if 'prediction' not in task:
        trainer.train()
    ...
    if 'prediction' in task or 'classification' in task:
        ...
        if task in ['eval_prediction_after_pretraining', 'zero_shot_prediction_from_another_dataset_pretraining']:
        ...
        basic_info = {
            ...
            'text_model': text_model_args.text_model_name,  # we train with our credit description data! did not use the openai pretrained one
            'freeze_text_params': model_config.freeze_text_params,
            ...
            'pretrain_model_final_ckpt_epoch': pretrain_model_final_ckpt_epoch if task == 'eval_prediction_after_pretraining' else None,
            'pretrain_steps_per_epoch': pretrain_steps_per_epoch if task == 'eval_prediction_after_pretraining' else None,
...
if __name__ == '__main__':
    parser = HfArgumentParser([TextModelArguments, MultimodalDataArguments, CrmmTrainingArguments])
